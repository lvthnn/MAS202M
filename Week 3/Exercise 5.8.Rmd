...
```{r message = FALSE}
hist_fn <- function(c) {
  plot <- ggplot(Weekly, aes(x = get(c))) + 
    geom_histogram() + 
    labs(
      x = paste(c),
      y = "Frequency"
...
grid.arrange(grobs = grobs, nrow = 3)
```
...
test  <- Weekly |> anti_join(train, by = join_by(Year))
```
...
train.X <- train |> select(-Direction)
train.Y <- train |> pull(Direction)

test.X <- test |> select(-Direction)
